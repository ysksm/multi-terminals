...
func (h *SetLastActivePaneHandler) Handle(ctx context.Context, cmd SetLastActivePaneCommand) error {
	...
	if err != nil {
		return apperr.Validation(fmt.Errorf("set last active pane: invalid workspace id: %w", err))
	}
	...
	if err != nil {
		return apperr.Validation(fmt.Errorf("set last active pane: invalid pane id: %w", err))
	}

	if err := w.SetLastActivePane(paneID); err != nil {
		return apperr.Validation(fmt.Errorf("set last active pane: %w", err))
	}
