...
| F-17 | サイドバー折りたたみ | トグルで開閉。状態を localStorage に保存し再読込後も維持 |

### 1.4 ターミナル（ランタイム）
| # | 機能 | 説明 |
| --- | --- | --- |
| F-18 | ワークスペースを開く | 全ペインに対し PTY 起動。`autoRun:true` の起動コマンドを順次送信。`lastOpened` を更新 |
| F-19 | 端末入出力 | xterm.js ↔ PTY の双方向ストリーム（入力↑ / 出力↓） |
...
- [ ] T3-5 `terminal.Runner`（go-pty 起動・LookPath・pump goroutine + 実シェルテスト）
...
- [ ] T5-6 `App.svelte`: サイドバー（一覧/作成/折りたたみ/2段階削除）
